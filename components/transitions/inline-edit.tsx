...
export function InlineEdit({
  ...
  const [localValue, setLocalValue] = useState(
    defaultValue ?? (preview ? "Website refresh" : ""),
  );
  ...
  useLayoutEffect(() => {
    const update = () => {
      if (!host.current || !measure.current || !content.current) return;
      ...
      const closed = Math.min(
        ...
        Math.max(
          112,
          Math.ceil(measure.current.getBoundingClientRect().width) + 56,
        ),
      );
  ...
  useLayoutEffect(() => {
    ...
    if (target?.getClientRects().length) {
      ...
      if (target === input.current) input.current?.select();
    }
  ...
  function edit() {
    if (!canEdit || pendingRevision.current !== null) return;
    revision.current += 1;
    if (!error) setDraft(displayed);
    setError("");
    setAnnouncement("");
    setSaved(false);
    ...
    Promise.resolve()
      ...
      .then(
        () => {
          ...
          setSaved(true);
          setAnnouncement(
            onCommit ? `${label} saved.` : `${label} updated in this preview.`,
          );
          ...
          >
            {editing ? (
              ...
              >
                ...
                />
                {error && (
                  <p className="bie-error" id={`${id}-error`} role="alert">
                    {error}
                  </p>
                )}
                <motion.div
                  className="bie-actions"
                ...
                >
                  ...
                  >
                    <motion.span
                      aria-hidden="true"
                      animate={{ rotate: pending && !reduced ? 360 : 0 }}
                      transition={
                        pending && !reduced
                          ? {
                              duration: 0.8 / rate,
              ...
              >
                ...
                {canEdit && (
                  <span className="bie-glyph" aria-hidden="true">
                    {pending ? (
                      <LoaderCircle size={15} />
                    ) : saved ? (
                      <Check size={15} />
