...
export function ConfirmAction({
  ...
  const [phase, setPhase] = useState<"idle" | "armed" | "pending" | "success">(
    "idle",
  );
  ...
  useLayoutEffect(() => {
    const update = () => {
      if (!host.current || !content.current || !measure.current) return;
      ...
      const closed = Math.min(
        ...
        Math.max(
          112,
          Math.ceil(measure.current.getBoundingClientRect().width) + 50,
        ),
      );
  ...
  function arm() {
    if (!available || phase !== "idle" || inFlight.current) return;
    setError("");
    focusTarget.current = "cancel";
    setPhase("armed");
  }
  ...
  function confirm() {
    if (phase !== "armed" || !available || inFlight.current) return;
    ...
    Promise.resolve()
      .then(() => {
        ...
        if (localPreview && revision.current === request) setArchived(true);
      })
      .then(
        () => {
  ...
  function restorePreview() {
    if (!localPreview || phase !== "success" || inFlight.current) return;
    setArchived(false);
    setError("");
    focusTarget.current = "trigger";
    setPhase("idle");
  ...
  return (
    ...
    >
      {preview && (
        <div className="bca-preview-heading">
          <div>
      ...
      <div className="bca-host" ref={host}>
        ...
        >
          <motion.div
            ref={content}
            className="bca-content"
            style={{ width: measured ? Math.max(0, width - 2) : undefined }}
            animate={presentation}
          >
            {open ? (
              <div
                className="bca-confirmation"
                role="group"
              ...
              >
                ...
                {error && (
                  <p className="bca-error" id={`${id}-error`} role="alert">
                    {error}
                  </p>
                )}
              </div>
              ...
              <button
                ref={trigger}
                type="button"
                className="bca-trigger"
                disabled={!available}
                aria-disabled={successful || undefined}
                onClick={arm}
              >
                {successful ? (
