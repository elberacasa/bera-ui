...
Read the selected entry in the [catalog](https://bera-ui.vercel.app/transitions/manifest.json), then follow its declared recipe and source URLs relative to https://bera-ui.vercel.app. Standalone `transitions` contain React and CSS; `adapters` declare the files needed to refine an existing primitive. Treat the source as an inspectable starting point. The entry's guidance defines its API and adaptation boundaries.
...
For an existing Radix dropdown, use the [Radix menu CSS adapter](https://bera-ui.vercel.app/adapters/radix-menu-motion.agent.md). It adds motion to the current content and submenu without requiring a new React component or Motion dependency. Preserve Radix's conditional mounting and finite exit lifecycle.
